// =========================================================
// ДЕМО-ЛОГИКА: брендирование, рендер услуг/мастеров/слотов,
// навигация по экранам, попапы Telegram (showPopup).
// telegram.js уже подключён ДО этого файла.
// =========================================================

// --- Текущий выбор пользователя в booking-флоу ---
...
function showAppPopup(title, message, buttons) {
  ...
  overlay.innerHTML = `
    <div style="background:var(--tg-bg,#fff);color:var(--tg-text,#1a1a1a);
      ...
      <div style="display:flex;gap:8px;justify-content:flex-end;">
        ${buttons.map(b =>
          `<button data-close style="padding:8px 16px;border-radius:10px;
            background:var(--brand-primary);color:#fff;border:none;font-weight:500;cursor:pointer;">${b.text}</button>`
        ).join("")}
      </div>
    </div>`;
...
function initBookButton() {
  ...
  btn.onclick = () => {
    ...
    showAppPopup(
      "✅ Запись подтверждена",
      `${selectedService.name}\n${selectedStaff.name} · сегодня, ${selectedSlot}\n\nНапоминание придёт автоматически за 2 часа до приема.`,
...
// ============================================
// 8. ИСТОРИЯ ВИЗИТОВ (вместо Vaqf)
// ============================================
function renderHistory() {
  const wrap = document.getElementById("history-content");
  if (!wrap) return;

  // Демо-данные: прошлые визиты
  const past = [
  ...
  past.forEach((visit, i) => {
    const card = document.createElement("div");
    card.style.cssText = `border:1px solid var(--tg-secondary-bg,#eee);
    ...
    card.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px;">
        <div>
          ...
          <p style="margin:3px 0 0;font-size:13px;color:var(--tg-hint,#999);">
            ${visit.staff.name} · ${visit.date}
          </p>
        </div>
      ...
      <button class="repeat-btn" data-idx="${i}" style="
  ...
  // Нажатие "Записаться снова" — предвыбирает услугу+мастера и кидает на экран записи
...
function showScreen(id) {
  ...
  document.querySelectorAll("#bottom-nav button").forEach(btn => {
    btn.style.color = (btn.dataset.screen === id)
      ? "var(--brand-primary)"
      : "var(--tg-hint,#999)";
  });
  if (typeof setBackButton === "function") {
    setBackButton(() => showScreen("booking"));
  }
}
